...
//Array di oggetti che rappresentano ogni sezione
const infoSections = [
  {
...
const InfoSection = () => {
  //Stato che tiene traccia dell'indice della sezione aperta (o null se nessuna è aperta)
  const [open, setOpen] = useState<number | null>(null);

  return (
    //Sezione principale che contiene tutte le sottoSezioni
    <section className="info-section">
      {/*Titolo della sezione*/}
      <h2 className="info-section-title">Guida alla Salute Mentale</h2>
      {/*Ciclo su tutte le sottoSezioni*/}
      {infoSections.map((s, i) => {
        //Verifica se la sezione corrente è aperta
        const isOpen = open === i;
        return (
          //Ogni sezione si puo' aprire e chiudere
          <div className={`info-item${isOpen ? " open" : ""}`} key={i}> {/*Crea un div per ogni sezione con la classe css info-item, aggiungendo la classe "open" se la sezione è aperta (per gestire lo stile e la visibilità del contenuto)*/}
            {/*Bottone per aprire/chiudere la sezione*/}
            <button
              className="info-title"
              onClick={() => setOpen(isOpen ? null : i)} //Se già aperta la chiude, altrimenti la apre
              aria-expanded={isOpen}
            >
              <span>{s.title}</span>
              {/*Freccia che ruota se la sezione è aperta*/}
              <span className={`arrow${isOpen ? " arrow-open" : ""}`}>▼</span>
            </button>
            {/*Contenuto della sezione, visibile solo se aperta*/}
